...
2. GPT-4o 语义匹配推荐 3 家，banner 显示每家推荐理由（含场景判断）
3. 地图金色 ⭐ marker 高亮推荐店，右侧卡片出现 AI PICK 标签，预算自动同步筛选器
4. 点击任意店铺名称 → 地图自动跳转到该店并显示蓝色定位圆环
5. 可叠加手动筛选：氛围、适合场景、主菜系均支持多选
6. 点击 **🚀 一键生成行程** → DBSCAN 地理聚类 + TSP 最短路径，地图画出彩色路线
7. 右侧展开路线详情（距离 + 步行 / 开车时间），点击 **🗺️ 在 Google Maps 看路线** 一键导出导航
